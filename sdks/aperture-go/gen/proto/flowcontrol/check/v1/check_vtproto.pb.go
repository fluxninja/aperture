...
func (m *CheckResponse) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.StateCache) > 0 {
		for k := range m.StateCache {
			v := m.StateCache[k]
			baseI := i
	...
	if m.ResultCache != nil {
		size, err := m.ResultCache.MarshalToSizedBufferVT(dAtA[:i])
		if err != nil {
			return 0, err
		}
		i -= size
...
func (m *CheckResponse) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 15:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ResultCache", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.ResultCache == nil {
				m.ResultCache = &KeyLookupResponse{}
			}
			if err := m.ResultCache.UnmarshalVT(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 16:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field StateCache", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.StateCache == nil {
				m.StateCache = make(map[string]*KeyLookupResponse)
			}
			var mapkey string
