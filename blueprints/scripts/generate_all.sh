...
# remove md files in "${blueprints_root}"/../docs/content/references/policies/bundled-blueprints and subdirectories
$FIND "${blueprints_root}"/../docs/content/references/policies/bundled-blueprints -type f -name '*.md' -delete
...
$FIND "$blueprints_root"/lib/1.0 -type f -name config.libsonnet | while read -r files; do
	...
	docs_dir="${blueprints_root}"/../docs/content/references/policies/bundled-blueprints/"$blueprint_dir"
	mkdir -p "$docs_dir"
	...
	# add the correct number of ../ to the import statement and additional 3 (references/policies/bundled-blueprints) levels to find apertureVersion.js
	import_levels="../../../"
