...
func (r *MutatingWebhookReconciler) reconcileMutatingWebhookConfiguration(ctx context.Context, instance *admissionregistrationv1.MutatingWebhookConfiguration) error {
	...
	if instance.GetName() == agentMutatingWebhookName {
...
func mwcEventFilters() predicate.Predicate {
	return predicate.Funcs{
		CreateFunc: func(create event.CreateEvent) bool {
			...
			return ok && (new.GetName() == agentMutatingWebhookName || new.GetName() == controllerMutatingWebhookName)
		},
		UpdateFunc: func(update event.UpdateEvent) bool {
			...
			if !ok1 || !ok2 {
				return false
			} else if new.GetName() != agentMutatingWebhookName && new.GetName() != controllerMutatingWebhookName {
				return false
			} else if old.GetName() != agentMutatingWebhookName && old.GetName() != controllerMutatingWebhookName {
				return false
			}
