...
while read -r app; do
	env_dir=$(dirname "$app")
	tk show --ext-str=ENABLE_CLOUD_EXTENSION=false --ext-str=ENABLE_DEMO_APP_RABBITMQ=false --ext-str=ENABLE_DEMO_APP_NGINX=false --ext-str=ENABLE_DEMO_APP_KONG=false --ext-str=SIMPLE_SERVICE_CPU_LOAD=0 --dangerous-allow-redirect --ext-str=projectRoot="${TOP_LEVEL}"/playground/tanka/ "$env_dir" >/dev/null || {
		exit_code="$?"
