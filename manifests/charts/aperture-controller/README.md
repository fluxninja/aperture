...
| `controller.config.fluxninja_plugin.enabled`                              | Boolean flag for enabling FluxNinja cloud connection from Controller                                                     | `false`  |
| `controller.config.fluxninja_plugin.fluxninja_endpoint`                   | FluxNinja cloud instance endpoint                                                                                        | `nil`    |
| `controller.config.fluxninja_plugin.heartbeat_interval`                   | specifies how often to send heartbeats to the cloud. Defaults to '30s'.                                                  | `30s`    |
| `controller.config.fluxninja_plugin.client_grpc.insecure`                 | specifies whether to communicate with FluxNinja cloud over TLS or in plain text using gRPC. Defaults to false.           | `false`  |
| `controller.config.fluxninja_plugin.client_grpc.tls.insecure_skip_verify` | specifies whether to verify FluxNinja cloud certificate using gRPC. Defaults to false.                                   | `false`  |
| `controller.config.fluxninja_plugin.client_grpc.tls.ca_file`              | specifies an alternative CA certificates bundle to use to validate FluxNinja cloud certificate using gRPC                | `nil`    |
| `controller.config.fluxninja_plugin.client_http.tls.insecure_skip_verify` | specifies whether to verify FluxNinja cloud certificate using HTTP. Defaults to false.                                   | `false`  |
| `controller.config.fluxninja_plugin.client_http.tls.ca_file`              | specifies an alternative CA certificates bundle to use to validate FluxNinja cloud certificate using HTTP                | `nil`    |
| `controller.config.server.addr`                                           | The Controller's server port. Defaults to 80.                                                                            | `:80`    |
| `controller.config.log.pretty_console`                                    | Additional log writer: pretty console (stdout) logging (not recommended for prod environments). Defaults to false.       | `false`  |
| `controller.config.log.non_blocking`                                      | Use non-blocking log writer (can lose logs at high throughput). Defaults to True.                                        | `true`   |
| `controller.config.log.level`                                             | Log level. Keywords allowed - ["debug", "info", "warn", "fatal", "panic", "trace"]. Defaults to 'info'.                  | `info`   |
| `controller.config.log.file`                                              | Output file for logs. Keywords allowed - ["stderr", "stderr", "default"]. Defaults to 'stderr'.                          | `stderr` |
...
| `controller.config.otel.grpc_addr`                                        | GRPC listener addr for OTEL Collector. Defaults to ":4317"                                                               | `:4317`  |
| `controller.config.otel.http_addr`                                        | HTTP listener addr for OTEL Collector. Defaults to ":4318"                                                               | `:4318`  |
| `controller.config.otel.batch_prerollup.send_batch_size`                  | SendBatchSize is the size of a batch which after hit, will trigger it to be sent. Defaults to "10000".                   | `10000`  |
| `controller.config.otel.batch_prerollup.timeout`                          | Timeout sets the time after which a batch will be sent regardless of size. Defaults to "1s".                             | `1s`     |
| `controller.config.otel.batch_postrollup.send_batch_size`                 | SendBatchSize is the size of a batch which after hit, will trigger it to be sent. Defaults to "10000".                   | `10000`  |
| `controller.config.otel.batch_postrollup.timeout`                         | Timeout sets the time after which a batch will be sent regardless of size. Defaults to "1s".                             | `1s`     |
