...
function(datasourceName, policyName, component, extraFilters={})
  ...
  local row1 = [
    statPanel(
      'Total Requests',
      datasourceName,
      'sum(increase(concurrency_limiter_counter_total{%(filters)s}[$__range]))' % { filters: stringFilters },
      x=0,
      h=10,
      w=8,
      panelColor='blue',
    ...
    statPanel(
      ...
      datasourceName,
      'sum(increase(concurrency_limiter_counter_total{%(filters)s, decision_type="DECISION_TYPE_ACCEPTED"}[$__range]))' % { filters: stringFilters },
      x=8,
      h=10,
      w=8,
      graphMode='area',
    ...
    statPanel(
      ...
      datasourceName,
      'sum(increase(concurrency_limiter_counter_total{%(filters)s, decision_type="DECISION_TYPE_REJECTED"}[$__range]))' % { filters: stringFilters },
      x=16,
      h=10,
      w=8,
      panelColor='red',
  ...
  local targets =
    [
      g.query.prometheus.new(datasourceName, 'sum by(decision_type) (rate(concurrency_limiter_counter_total{%(filters)s}[$__rate_interval]))' % { filters: stringFilters })
