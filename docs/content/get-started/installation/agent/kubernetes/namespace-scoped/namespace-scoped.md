...
2. If you want to install the Aperture Agent using `Helm` or don't want to use
   ...
   the client SSL certificate for the same in `values.yaml` file as below:
...
1. Configure the below parameters of Etcd and Prometheus for the Agent by
   updating the `values.yaml` and pass it with `install` command:
...
2. If you want to modify the default parameters or the Aperture Agent config,
   for example `log`, you can update the `values.yaml` file and pass it with
   `install` command:
