...
```mdx-code-block
<CodeBlock language="bash">aperturectl blueprints values --name=policies/load-scheduling/average-latecy --version={apertureVersion} --output-file=values.yaml</CodeBlock>
```
...
```mdx-code-block
<CodeBlock language="bash">aperturectl blueprints generate --name=policies/load-scheduling/average-latecy
...
```bash
kubectl apply -f policy-gen/configuration/load-scheduling/average-latency -n aperture-controller
```
