...
When the service is recovering from an overload state, the load multiplier is
gradually increased in each execution tick. This parameter determines the
maximum load multiplier that can be reached during the recovery. This parameter
helps protect the service from request bursts while the system is still
recovering. Once this value is reached, the scheduler enters the pass through
mode, i.e. The requests bypass the scheduler and are sent directly to the
service. Any future overload state is detected by the control policy and the
