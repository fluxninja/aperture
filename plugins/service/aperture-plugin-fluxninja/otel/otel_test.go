...
var _ = DescribeTable("FN Plugin OTEL", func(
	...
	hb := &heartbeats.Heartbeats{}
	hb.SetControllerInfo(&heartbeatv1.ControllerInfo{
		Id: "controllero",
	})
	...
	startCtx, startCancal := context.WithTimeout(context.Background(), 60*time.Second)
	defer startCancal()
	err = app.Start(startCtx)
	Expect(err).NotTo(HaveOccurred())

	stopCtx, stopCancal := context.WithTimeout(context.Background(), 60*time.Second)
	defer stopCancal()
