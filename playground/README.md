...
> 📍 Verify that nothing else is running on the
> [ports forwarded](#port-forwards) by `Tilt`.
...
[Rate-Limiting Actuator](/concepts/flow-control/rate-limiter.md) to dynamically
rate-limit traffic from unwanted users, which protects the demo application
against sudden surges in traffic load. You can verify it using the following
command:
...
![Stop Wavepool Generator](./assets/stop-traffic.png)
...
> 📍 Please skip this section in case you already installed the required tools
> using [`asdf`](#install-using-asdf).
...
> 📍 You can skip this section if you already have a running cluster which is
...
> that case, the latest images will be pulled from DockerHub and used instead.
