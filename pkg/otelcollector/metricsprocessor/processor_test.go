...
func expectEngineCalls(engine *mocks.MockEngine, checkResponse *flowcontrolv1.CheckResponse) {
	...
	for i, fm := range checkResponse.FluxMeters {
		...
		expectedCalls[i] = engine.EXPECT().GetFluxMeterHist(fm.GetPolicyName(), fm.GetFluxMeterName(), fm.GetPolicyHash(), "200", flowcontrolv1.DecisionType_DECISION_TYPE_REJECTED).Return(nil)
	}
