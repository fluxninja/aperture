...
func (textTracesMarshaler) MarshalTraces(td ptrace.Traces) ([]byte, error) {
	...
	for i := 0; i < rss.Len(); i++ {
		...
		buf.logMap("Resource labels", rs.Resource().Attributes())
		ilss := rs.ScopeSpans()
		for j := 0; j < ilss.Len(); j++ {
			buf.logEntry("ScopeSpans #%d", j)
			ils := ilss.At(j)
			buf.logEntry("ScopeMetrics SchemaURL: %s", ils.SchemaUrl())
			buf.logScope(ils.Scope())
			...
			for k := 0; k < spans.Len(); k++ {
				...
				buf.logMap("Attributes", span.Attributes())
