...
func (b *dataBuffer) logMap(label string, am pcommon.Map) {
	if am.Len() == 0 {
		return
	}
...
func (b *dataBuffer) logScope(il pcommon.InstrumentationScope) {
	b.logEntry(
		"Scope %s %s",
		il.Name(),
		il.Version())
...
func (b *dataBuffer) logDoubleHistogramDataPoints(ps pmetric.HistogramDataPointSlice) {
	for i := 0; i < ps.Len(); i++ {
		...
		buckets := p.MExplicitBounds()
		if len(buckets) != 0 {
			for j, bucket := range buckets {
				b.logEntry("Buckets #%d, Count: %f", j, bucket)
			}
		}
	}
}
...
func (b *dataBuffer) logDataPointAttributes(labels pcommon.Map) {
	b.logMap("Data point attributes", labels)
}
...
func attributeValueSliceToString(av pcommon.Slice) string {
