...
var _ = Describe("Rollup processor", func() {
	...
	Describe("Traces", func() {
		...
		It("works for single trace record", func() {
			...
			spanRecord.Attributes().InsertString(otelcollector.WorkloadDurationLabel, strconv.Itoa(attributeValues[0]))
		...
		It("works for multiple trace records", func() {
			...
			spanRecord.Attributes().InsertString(otelcollector.WorkloadDurationLabel, strconv.Itoa(attributeValues[0]))
			spanRecord = spans.AppendEmpty()
			spanRecord.Attributes().InsertString(otelcollector.WorkloadDurationLabel, strconv.Itoa(attributeValues[1]))
			spanRecord = spans.AppendEmpty()
			spanRecord.Attributes().InsertString(otelcollector.WorkloadDurationLabel, strconv.Itoa(attributeValues[2]))
