    ...
    - query:
        promql:
          ...
          out_ports:
            output:
              signal_name: SIGNAL
          query_string: sum(increase(flux_meter_sum{flow_status="OK", flux_meter_name="service1-demo-app"}[15s]))/sum(increase(flux_meter_count{flow_status="OK",
            flux_meter_name="service1-demo-app"}[15s]))
