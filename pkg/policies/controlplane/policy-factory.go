...
func providePolicyFactory(
	...
	policiesStatusRegistry := registry.ChildKV("system", iface.PoliciesRoot)
	logger := policiesStatusRegistry.GetLogger()

	circuitJobGroup, err := jobs.NewJobGroup(policiesStatusRegistry.ChildKV("policy", "circuit_jobs"), 0, jobs.RescheduleMode, nil)
	if err != nil {
