...
func setupRateLimiterFactory(
	...
	reg := statusRegistry.ChildKV("component", rateLimiterStatusRoot)
	logger := reg.GetLogger()

	lazySyncJobGroup, err := jobs.NewJobGroup(reg.ChildKV("sync", "lazy_sync_jobs"), 0, jobs.RescheduleMode, nil)
	if err != nil {
