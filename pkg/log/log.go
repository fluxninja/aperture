...
const (
	...
	sampledKey = "sampled"
	// SampledKey is a field key that is used for Bug() events.
	bugKey = "bug"
)
...
func Autosample() *Logger {
	// Note: not calling global.Sample() as it'd mess up with caller depth in
	// getAutosampler().
...
func Bug() *zerolog.Event {
	// Note: not calling global.Bug() as it'd mess up with caller depth in
	// getAutosampler().
