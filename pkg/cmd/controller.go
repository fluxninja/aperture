...
func (h *Handler) ListControlPoints(
...
) (*cmdv1.ListFlowControlControlPointsControllerResponse, error) {
	...
	for _, resp := range agentsControlPoints {
		...
		for _, protoCp := range resp.Success.FlowControlControlPoints.FlowControlControlPoints {
			gcp := selectors.ControlPointIDFromProto(protoCp).InAgentGroup(resp.Success.AgentGroup)
			allControlPoints[gcp] = struct{}{}
		}
	}

	protoControlPoints := make([]*cmdv1.GlobalFlowControlControlPoint, 0, len(allControlPoints))
	for cp := range allControlPoints {
		protoControlPoints = append(protoControlPoints, cp.ToProto())
	}

	return &cmdv1.ListFlowControlControlPointsControllerResponse{
		GlobalFlowControlControlPoints: protoControlPoints,
...
agentsLoop:
	for _, agent := range agentsControlPoints {
		...
		for _, cpProto := range agent.Success.FlowControlControlPoints.FlowControlControlPoints {
